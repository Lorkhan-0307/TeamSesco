...
def run(
        ...
        name='modified_exp',  # save results to project/name
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            danger_path = str(save_dir / 'labels' / p.stem)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        # 라벨 위치!
                        if (xywh[2] * xywh[3] >= 0.3):
                            count += 1
                            label = None if hide_labels else (names[c] if hide_conf else f'{names[c]} {conf:.2f}')
                            annotator.box_label(xyxy, label, color=colors(c, True))
                            tempdangerlist.append(label[:-5])
                ...
                if(tempdangerlist != danger_list):
                    ...
                    danger_list = tempdangerlist
                tempdangerlist.clear()
                print(tempdangerlist)


            if(not webcam):

                if(count >0 and record == False):
                    LOGGER.info(f"###################################3Start recording danger")
                    record = True
                    video = cv2.VideoWriter("C:/Users/Public/Capstone_AIHackathon/yolov5/runs/savedanger/" + str((datetime.datetime.now().strftime("%F_%H-%M-%S"))) + ".mp4",cv2.VideoWriter_fourcc(*'mp4v'),
                                        10, (im0.shape[1], im0.shape[0]))

                if(record==True and count == 0):
                    record = False
                    LOGGER.info(f"#############################Fin recording danger")
                    video.release()
